...
// Punto de recolección para pedidos sin envío
const PICKUP_LOCATION = {
    name: 'TrippGo',
    addressLines: [
        'Manuel Vargas Cataño S/N, Local #3',
        'Col. San Antonio, C.P. 42083',
        'Pachuca de Soto, Hidalgo'
    ],
    mapsUrl: 'https://maps.app.goo.gl/H5Zh5Md2tULEDGWa6',
    hours: [
        'Lunes a Viernes: 10:30 am a 5:30 pm',
        'Sábado: 10:30 am a 3:30 pm'
    ]
};

// Etiquetas legibles de estado (para asuntos y badges de correo)
const STATUS_LABELS = {
    'pagado': 'PAGADO',
    'procesado': 'PROCESADO',
    'enviado': 'ENVIADO',
...
// Bloque HTML con la ubicación, horarios y aviso del punto de recolección
function pickupLocationHtml() {
    return `
        <div style="background: #f9f9f9; border-left: 4px solid #F5A84F; border-radius: 8px; padding: 16px; margin: 20px 0;">
            <p style="margin: 0 0 8px; font-weight: 700; color: #1a1a1a;">📍 Punto de recolección — ${PICKUP_LOCATION.name}</p>
            ${PICKUP_LOCATION.addressLines.map(l => `<p style="margin: 2px 0; color: #666;">${l}</p>`).join('')}
            <p style="margin: 14px 0 4px; font-weight: 600; color: #1a1a1a;">Horario:</p>
            ${PICKUP_LOCATION.hours.map(h => `<p style="margin: 2px 0; color: #666;">${h}</p>`).join('')}
            <p style="margin: 14px 0 0;">
                <a href="${PICKUP_LOCATION.mapsUrl}" target="_blank" style="background: #F5A84F; color: #fff; padding: 10px 18px; border-radius: 6px; text-decoration: none; font-weight: 600; display: inline-block;">Abrir en Google Maps</a>
            </p>
            <p style="margin: 16px 0 0; color: #888; font-size: 13px;">
                <strong>Importante:</strong> tu pedido estará disponible durante 5 días hábiles en el punto de entrega.
                Posterior a ese período, si aún no lo has recolectado, el pedido será devuelto a nuestro almacén
                y tendremos que reagendar su recolección.
            </p>
        </div>`;
}
...
async function sendOrderConfirmation(order) {
    ...
    <div style="font-family: 'Helvetica Neue', Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #333;">
        ...
        <div style="padding: 30px; background: #fff;">
            ...
            <p>¡Hemos recibido tu pedido con éxito! Lo procesaremos al siguiente día hábil de la fecha de tu compra. Te recomendamos mantenerte al tanto de las actualizaciones que te estaremos enviando por correo electrónico para que puedas hacer el seguimiento de tu pedido.</p>

            <div style="background: #f9f9f9; border-radius: 8px; padding: 16px; margin: 20px 0;">
                <p style="margin: 0; font-size: 14px; color: #888;">Número de pedido</p>
                <p style="margin: 4px 0 0; font-size: 20px; font-weight: 700; color: #F5A84F;">${formatOrderId(order)}</p>
            </div>
            ...
            ${pickup ? `
            <div style="background: #f9f9f9; border-radius: 8px; padding: 16px; margin: 20px 0;">
                <p style="margin: 0 0 4px; font-weight: 600;">Método de entrega: Recolección en punto de entrega</p>
                <p style="margin: 0; color: #666;">Te enviaremos la ubicación y los horarios por correo en cuanto tu pedido esté listo para recolección.</p>
            </div>
            ` : (addressStr ? `
            <div style="background: #f9f9f9; border-radius: 8px; padding: 16px; margin: 20px 0;">
    ...
    try {
        await resend.emails.send({
            ...
            subject: `Confirmación de pedido ${formatOrderId(order)} - Legado San José`,
            html
        });
...
async function sendOrderNotification(order) {
    ...
    try {
        await resend.emails.send({
            from: FROM_EMAIL,
            to: [BUSINESS_EMAIL],
            subject: `Nueva venta: ${formatOrderId(order)} - $${(order.total || 0).toLocaleString('es-MX')} MXN`,
            html
        });
...
async function sendStatusUpdate(order, newStatus) {
    ...
    // Mensaje y contenido extra según el estado y método de entrega
    let message = '';
    let extraHtml = '';
    ...
    if (pickup) {
        ...
        const pickupMessages = {
            'procesado': 'Hemos procesado tu pedido y lo estamos preparando en nuestro almacén. Pronto te notificaremos cuando esté listo para recolección en nuestro punto de entrega.',
            'listo_para_recoleccion': `Tu pedido está listo para recolección en ${PICKUP_LOCATION.name}. Consulta la ubicación y los horarios de atención:`,
            'entregado': '¡Hemos entregado tu pedido! Gracias por tu compra, esperamos que disfrutes llevando el legado contigo.'
        };
        message = pickupMessages[newStatus];
        if (newStatus === 'listo_para_recoleccion') {
            extraHtml = pickupLocationHtml();
        }
    } else {
        // Pedidos con envío: construir info de rastreo para "enviado"
        let trackingHtml = '';
        let enviadoMessage = '';
        if (newStatus === 'enviado' && order.tracking_number) {
            const carrierName = order.shipping_carrier ? ` vía ${order.shipping_carrier}` : '';
            if (order.tracking_url) {
                trackingHtml = `
                <div style="background: #f9f9f9; border-radius: 8px; padding: 16px; margin: 16px 0; text-align: center;">
                    <p style="margin: 0 0 8px; color: #888; font-size: 13px;">Número de rastreo</p>
                    <a href="${order.tracking_url}" target="_blank" style="color: #F5A84F; font-weight: 700; font-size: 16px; text-decoration: none;">${order.tracking_number}</a>
                    <p style="margin: 8px 0 0;"><a href="${order.tracking_url}" target="_blank" style="color: #F5A84F; font-size: 13px;">Rastrear mi pedido →</a></p>
                </div>`;
            } else {
                trackingHtml = `<p style="margin-top: 8px;"><strong>Número de rastreo:</strong> ${order.tracking_number}</p>`;
            }
            enviadoMessage = `Tu pedido ha sido enviado${carrierName}.`;
        }

        const shippingMessages = {
            'procesado': 'Hemos procesado tu pedido y lo estamos preparando para su envío.',
            'enviado': enviadoMessage || `Tu pedido ha sido enviado.${order.tracking_number ? ' Número de rastreo: ' + order.tracking_number : ''}`,
            'entregado': '¡Hemos entregado tu pedido! Gracias por tu compra, esperamos que disfrutes llevando el legado contigo.'
        };
        message = shippingMessages[newStatus];
        extraHtml = trackingHtml;
    }

    if (!message) return;

    const statusLabel = STATUS_LABELS[newStatus] || newStatus.toUpperCase();
    ...
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="padding: 30px;">
            ...
            <div style="background: #f9f9f9; border-radius: 8px; padding: 20px; margin: 20px 0; text-align: center;">
                ...
                <p style="margin: 8px 0 0; font-size: 18px; font-weight: 700; color: #F5A84F;">${statusLabel}</p>
            </div>
    ...
    try {
        await resend.emails.send({
            ...
            to: [customerEmail],
            reply_to: REPLY_TO_EMAIL,
            subject: `Tu pedido ${formatOrderId(order)} - ${statusLabel} | Legado San José`,
            html
        });
