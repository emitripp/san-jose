...
async function sendOrderNotification(order) {
    ...
    const itemsList = (order.items || []).map(item =>
        `• ${item.name}${item.size ? ' (' + item.size + ')' : ''} x${item.quantity || 1} — $${(item.price || 0).toLocaleString('es-MX')} MXN`
    ).join('<br>');

    const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #F5A84F;">Nueva Venta Recibida</h2>

        <div style="background: #f9f9f9; padding: 16px; border-radius: 8px; margin: 16px 0;">
            <p><strong>Pedido:</strong> ${order.id}</p>
            <p><strong>Cliente:</strong> ${order.customer?.name || 'N/A'}</p>
            <p><strong>Email:</strong> ${order.customer?.email || 'N/A'}</p>
            <p><strong>Total:</strong> $${(order.total || 0).toLocaleString('es-MX')} MXN</p>
            ${order.shipping ? `<p><strong>Envío:</strong> $${order.shipping.toLocaleString('es-MX')} MXN</p>` : ''}
        </div>
        ...
        <p style="margin-top: 20px;">
            <a href="${BASE_URL}/admin/" style="background: #F5A84F; color: #fff; padding: 10px 20px; border-radius: 6px; text-decoration: none; font-weight: 600;">
                Ver en Admin Panel
            </a>
        </p>
    </div>
...
async function sendStatusUpdate(order, newStatus) {
    ...
    const statusMessages = {
        ...
        'enviado': `Tu pedido ha sido enviado.${order.trackingNumber ? ' Número de rastreo: ' + order.trackingNumber : ''}`,
    ...
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="padding: 30px;">
            ...
            <div style="background: #f9f9f9; border-radius: 8px; padding: 20px; margin: 20px 0; text-align: center;">
                <p style="margin: 0; color: #888; font-size: 14px;">Pedido ${order.id}</p>
