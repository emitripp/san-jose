 ...
 */
async function generateLabel(rateId, destination, parcels, declaredValue = 0) {
    ...
    const created = await apiRequest('POST', '/api/v1/shipments', {
        shipment: {
            rate_id: rateId,
            ...
            address_to: {
                ...
                reference: destination.street2 || 'Destino'
            },
 ...
 */
async function pollShipment(shipmentId) {
    const maxWait = 60000;
    const pollInterval = 2500;
    const startTime = Date.now();
    ...
    while (Date.now() - startTime < maxWait) {
        ...
        const pkg = included.find(i => i.type === 'packages' && i.attributes?.label_url)
            || included.find(i => i.type === 'packages');
        const pkgAttrs = pkg?.attributes || {};
        lastStatus = attrs.workflow_status || lastStatus;
    ...
    throw new Error(`La guía sigue en proceso en Skydropx (estado: ${lastStatus}). Intenta de nuevo en unos minutos; el envío ${shipmentId} ya fue creado.`);
}
